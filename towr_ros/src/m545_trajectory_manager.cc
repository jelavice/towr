...
#include <towr_ros/m545_trajectory_manager.h>
    ...
    const SplineHolder &solution)
{
  ...
  while (t <= T + 1e-5) {
    ...
    for (int i = 0; i < n_ee; ++i){
      state.joint_positions_.at(i) = solution.as<SplineHolderExtended>()->joint_motion_.at(i)->GetPoint(t).p();
    }

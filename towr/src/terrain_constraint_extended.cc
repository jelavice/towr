...
TerrainConstraintExtended::TerrainConstraintExtended (const HeightMap::Ptr& terrain,
                                      std::string ee_motion, int ee_id, bool ee_has_wheel)
...
void
TerrainConstraintExtended::InitVariableDependedQuantities (const VariablesPtr& x)
{
  ...
  if (ee_has_wheel_ == false)
    ...
    ee_with_wheel_motion_ = x->GetComponent<NodesVariablesAll>(ee_motion_id_);
...
Eigen::VectorXd
TerrainConstraintExtended::GetValues () const
{
  VectorXd g(GetRows());
...
TerrainConstraintExtended::VecBound
TerrainConstraintExtended::GetBounds () const
{
  VecBound bounds(GetRows());
  double max_distance_above_terrain = 1e20; // [m]

  //todo in order to implement this, I need to add bool keeping in the time discretized nodes
  // Need to know whether they are in contact or not
  int row = 0;
  for (int id : node_ids_) {
...
void
TerrainConstraintExtended::FillJacobianBlock (std::string var_set, Jacobian& jac) const
{
